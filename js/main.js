...
if (fg && Array.isArray(FACTIONS)) {
  FACTIONS.forEach(f => {
    ...
    el.innerHTML = `
      ...
      <button class="fc-cta" style="background:${f.color}18;border-color:${f.color}30;" onclick="openModal('${f.key}')">
        Złóż Podanie →
      </button>
    `;
    fg.appendChild(el);
...
if (glow) {
  window.addEventListener("mousemove", e => {
    glow.animate({ left: `${e.clientX}px`, top: `${e.clientY}px` }, { duration: 300, fill: "forwards" });
  });
}
...
function updateAuthUI() {
  const user     = getUser(); // z auth.js — synchroniczne, ale już po authReady
  ...
  if (userBox) {
    ...
    logoutBtn?.addEventListener("click", async e => {
      e.stopPropagation();
      await logout(); // auth.js — wywoła triggerAuthUpdate sam
    });
  }
}

// Czekamy na auth:update które auth.js odpala PO fetchCurrentUser()
// Dzięki temu getUser() jest już ustawiony gdy tu trafiamy
window.addEventListener("auth:update", updateAuthUI);

// Bezpieczny fallback — jeśli authReady jest już resolved zanim tu dotarliśmy
authReady.then(updateAuthUI);
  ...
  function animate() {
    ...
    for (const p of particles) {
      if (mouse.x !== null) {
        ...
        if (d < 200) { const f = (200-d)/200; p.fx += (dx/d)*f*1.5; p.fy += (dy/d)*f*1.5; }
      }
      p.x += p.dx + p.fx; p.y += p.dy + p.fy;
      p.fx *= 0.92; p.fy *= 0.92;
...
/* ================= STATYSTYKI SERWERA (proxy backend) ================= */
async function updateServerStats() {
  ...
  try {
    ...
    if (listEl) listEl.innerHTML = players.map(p => `<div class="player-item">${p.name}</div>`).join('');
  } catch (err) {
...
function animateCounter(element, target) {
  if (!element) return;
  const duration = 1500, start = parseInt(element.innerText) || 0, startTime = performance.now();
  function update(t) {
    const progress = Math.min((t - startTime) / duration, 1);
    element.innerText = Math.floor(start + (target - start) * (1 - Math.pow(1 - progress, 3)));
